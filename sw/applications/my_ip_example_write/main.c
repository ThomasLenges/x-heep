#include <stdio.h>
#include <stdint.h>
#include <stdlib.h>

#include "core_v_mini_mcu.h"
#include "x-heep.h"
#include "w25q128jw.h"

#include "my_ip_regs.h"
#include "my_ip_structs.h"

#include "dma.h"

// what to write in flash
uint32_t flash_write_data[1025] = {
    0x76543211, 0xfedcba99, 0x579a6f91, 0x657d5bef, 0x758ee420, 0x01234568, 0xfedbca97, 0x89abde00,
    0x76543212, 0xfedcba9a, 0x579a6f92, 0x657d5bf0, 0x758ee421, 0x01234569, 0xfedbca98, 0x89abde01,
    0x76543213, 0xfedcba9b, 0x579a6f93, 0x657d5bf1, 0x758ee422, 0x0123456a, 0xfedbca99, 0x89abde02,
    0x76543214, 0xfedcba9c, 0x579a6f94, 0x657d5bf2, 0x758ee423, 0x0123456b, 0xfedbca9a, 0x89abde03,
    0x76543215, 0xfedcba9d, 0x579a6f95, 0x657d5bf3, 0x758ee424, 0x0123456c, 0xfedbca9b, 0x89abde04,
    0x76543216, 0xfedcba9e, 0x579a6f96, 0x657d5bf4, 0x758ee425, 0x0123456d, 0xfedbca9c, 0x89abde05,
    0x76543217, 0xfedcba9f, 0x579a6f97, 0x657d5bf5, 0x758ee426, 0x0123456e, 0xfedbca9d, 0x89abde06,
    0x76543218, 0xfedcbaa0, 0x579a6f98, 0x657d5bf6, 0x758ee427, 0x0123456f, 0xfedbca9e, 0x89abde07,

    0x76543219, 0xfedcbaa1, 0x579a6f99, 0x657d5bf7, 0x758ee428, 0x01234570, 0xfedbca9f, 0x89abde08,
    0x7654321a, 0xfedcbaa2, 0x579a6f9a, 0x657d5bf8, 0x758ee429, 0x01234571, 0xfedbcaa0, 0x89abde09,
    0x7654321b, 0xfedcbaa3, 0x579a6f9b, 0x657d5bf9, 0x758ee42a, 0x01234572, 0xfedbcaa1, 0x89abde0a,
    0x7654321c, 0xfedcbaa4, 0x579a6f9c, 0x657d5bfa, 0x758ee42b, 0x01234573, 0xfedbcaa2, 0x89abde0b,
    0x7654321d, 0xfedcbaa5, 0x579a6f9d, 0x657d5bfb, 0x758ee42c, 0x01234574, 0xfedbcaa3, 0x89abde0c,
    0x7654321e, 0xfedcbaa6, 0x579a6f9e, 0x657d5bfc, 0x758ee42d, 0x01234575, 0xfedbcaa4, 0x89abde0d,
    0x7654321f, 0xfedcbaa7, 0x579a6f9f, 0x657d5bfd, 0x758ee42e, 0x01234576, 0xfedbcaa5, 0x89abde0e,
    0x76543220, 0xfedcbaa8, 0x579a6fa0, 0x657d5bfe, 0x758ee42f, 0x01234577, 0xfedbcaa6, 0x89abde0f,

    0x76543221, 0xfedcbaa9, 0x579a6fa1, 0x657d5bff, 0x758ee430, 0x01234578, 0xfadbcaa7, 0x89abde10,
    0x76543222, 0xfedcbaaa, 0x579a6fa2, 0x657d5c00, 0x758ee431, 0x01234579, 0xfadbcaa8, 0x89abde11,
    0x76543223, 0xfedcbaab, 0x579a6fa3, 0x657d5c01, 0x758ee432, 0x0123457a, 0xfadbcaa9, 0x89abde12,
    0x76543224, 0xfedcbaac, 0x579a6fa4, 0x657d5c02, 0x758ee433, 0x0123457b, 0xfadbcaaa, 0x89abde13,
    0x76543225, 0xfedcbaad, 0x579a6fa5, 0x657d5c03, 0x758ee434, 0x0123457c, 0xfadbcaab, 0x89abde14,
    0x76543226, 0xfedcbaae, 0x579a6fa6, 0x657d5c04, 0x758ee435, 0x0123457d, 0xfadbcaac, 0x89abde15,
    0x76543227, 0xfedcbaaf, 0x579a6fa7, 0x657d5c05, 0x758ee436, 0x0123457e, 0xfadbcaad, 0x89abde16,
    0x76543228, 0xfedcbab0, 0x579a6fa8, 0x657d5c06, 0x758ee437, 0x0123457f, 0xfadbcaae, 0x89abde17,

    0x76543229, 0xfedcbab1, 0x579a6fa9, 0x657d5c07, 0x758ee438, 0x01234580, 0xfadbcab0, 0x89abde18,
    0x7654322a, 0xfedcbab2, 0x579a6faa, 0x657d5c08, 0x758ee439, 0x01234581, 0xfadbcab1, 0x89abde19,
    0x7654322b, 0xfedcbab3, 0x579a6fab, 0x657d5c09, 0x758ee43a, 0x01234582, 0xfadbcab2, 0x89abde1a,
    0x7654322c, 0xfedcbab4, 0x579a6fac, 0x657d5c0a, 0x758ee43b, 0x01234583, 0xfadbcab3, 0x89abde1b,
    0x7654322d, 0xfedcbab5, 0x579a6fad, 0x657d5c0b, 0x758ee43c, 0x01234584, 0xfadbcab4, 0x89abde1c,
    0x7654322e, 0xfedcbab6, 0x579a6fae, 0x657d5c0c, 0x758ee43d, 0x01234585, 0xfadbcab5, 0x89abde1d,
    0x7654322f, 0xfedcbab7, 0x579a6faf, 0x657d5c0d, 0x758ee43e, 0x01234586, 0xfadbcab6, 0x89abde1e,
    0x76543230, 0xfedcbab8, 0x579a6fb0, 0x657d5c0e, 0x758ee43f, 0x01234587, 0xfadbcab7, 0x89abde1f,

    0x76543231, 0xfedcbab9, 0x579a6fb1, 0x657d5c0f, 0x758ee440, 0x01234588, 0xfadbcab8, 0x89abde20,
    0x76543232, 0xfedcbaba, 0x579a6fb2, 0x657d5c10, 0x758ee441, 0x01234589, 0xfadbcab9, 0x89abde21,
    0x76543233, 0xfedcbabb, 0x579a6fb3, 0x657d5c11, 0x758ee442, 0x0123458a, 0xfadbcaba, 0x89abde22,
    0x76543234, 0xfedcbabc, 0x579a6fb4, 0x657d5c12, 0x758ee443, 0x0123458b, 0xfadbcabb, 0x89abde23,
    0x76543235, 0xfedcbabd, 0x579a6fb5, 0x657d5c13, 0x758ee444, 0x0123458c, 0xfadbcabc, 0x89abde24,
    0x76543236, 0xfedcbabe, 0x579a6fb6, 0x657d5c14, 0x758ee445, 0x0123458d, 0xfadbcabd, 0x89abde25,
    0x76543237, 0xfedcbabf, 0x579a6fb7, 0x657d5c15, 0x758ee446, 0x0123458e, 0xfadbcabe, 0x89abde26,
    0x76543238, 0xfedcbac0, 0x579a6fb8, 0x657d5c16, 0x758ee447, 0x0123458f, 0xfadbcabf, 0x89abde27,

    0x76543239, 0xfedcbac1, 0x579a6fb9, 0x657d5c17, 0x758ee448, 0x01234590, 0xfadbcac0, 0x89abde28,
    0x7654323a, 0xfedcbac2, 0x579a6fba, 0x657d5c18, 0x758ee449, 0x01234591, 0xfadbcac1, 0x89abde29,
    0x7654323b, 0xfedcbac3, 0x579a6fbb, 0x657d5c19, 0x758ee44a, 0x01234592, 0xfadbcac2, 0x89abde2a,
    0x7654323c, 0xfedcbac4, 0x579a6fbc, 0x657d5c1a, 0x758ee44b, 0x01234593, 0xfadbcac3, 0x89abde2b,
    0x7654323d, 0xfedcbac5, 0x579a6fbd, 0x657d5c1b, 0x758ee44c, 0x01234594, 0xfadbcac4, 0x89abde2c,
    0x7654323e, 0xfedcbac6, 0x579a6fbe, 0x657d5c1c, 0x758ee44d, 0x01234595, 0xfadbcac5, 0x89abde2d,
    0x7654323f, 0xfedcbac7, 0x579a6fbf, 0x657d5c1d, 0x758ee44e, 0x01234596, 0xfadbcac6, 0x89abde2e,
    0x76543240, 0xfedcbac8, 0x579a6fc0, 0x657d5c1e, 0x758ee44f, 0x01234597, 0xfadbcac7, 0x89abde2f,

    0x76543241, 0xfedcbac9, 0x579a6fc1, 0x657d5c1f, 0x758ee450, 0x01234598, 0xfadbcac8, 0x89abde30,
    0x76543242, 0xfedcbaca, 0x579a6fc2, 0x657d5c20, 0x758ee451, 0x01234599, 0xfadbcac9, 0x89abde31,
    0x76543243, 0xfedcbacb, 0x579a6fc3, 0x657d5c21, 0x758ee452, 0x0123459a, 0xfadbcaca, 0x89abde32,
    0x76543244, 0xfedcbacc, 0x579a6fc4, 0x657d5c22, 0x758ee453, 0x0123459b, 0xfadbcacb, 0x89abde33,
    0x76543245, 0xfedcbacd, 0x579a6fc5, 0x657d5c23, 0x758ee454, 0x0123459c, 0xfadbcacc, 0x89abde34,
    0x76543246, 0xfedcbace, 0x579a6fc6, 0x657d5c24, 0x758ee455, 0x0123459d, 0xfadbcacd, 0x89abde35,
    0x76543247, 0xfedcbacf, 0x579a6fc7, 0x657d5c25, 0x758ee456, 0x0123459e, 0xfadbcace, 0x89abde36,
    0x76543248, 0xfedcbad0, 0x579a6fc8, 0x657d5c26, 0x758ee457, 0x0123459f, 0xfadbcacf, 0x89abde37,

    0x76543249, 0xfedcbad1, 0x579a6fc9, 0x657d5c27, 0x758ee458, 0x012345a0, 0xfadbcad0, 0x89abde38,
    0x7654324a, 0xfedcbad2, 0x579a6fca, 0x657d5c28, 0x758ee459, 0x012345a1, 0xfadbcad1, 0x89abde39,
    0x7654324b, 0xfedcbad3, 0x579a6fcb, 0x657d5c29, 0x758ee45a, 0x012345a2, 0xfadbcad2, 0x89abde3a,
    0x7654324c, 0xfedcbad4, 0x579a6fcc, 0x657d5c2a, 0x758ee45b, 0x012345a3, 0xfadbcad3, 0x89abde3b,
    0x7654324d, 0xfedcbad5, 0x579a6fcd, 0x657d5c2b, 0x758ee45c, 0x012345a4, 0xfadbcad4, 0x89abde3c,
    0x7654324e, 0xfedcbad6, 0x579a6fce, 0x657d5c2c, 0x758ee45d, 0x012345a5, 0xfadbcad5, 0x89abde3d,
    0x7654324f, 0xfedcbad7, 0x579a6fcf, 0x657d5c2d, 0x758ee45e, 0x012345a6, 0xfadbcad6, 0x89abde3e,
    0x76543250, 0xfedcbad8, 0x579a6fd0, 0x657d5c2e, 0x758ee45f, 0x012345a7, 0xfadbcad7, 0x89abde3f,

    0x76543211, 0xfedcba99, 0x579a6f91, 0x657d5bef, 0x758ee420, 0x01234568, 0xfedbca97, 0x89abde00,
    0x76543212, 0xfedcba9a, 0x579a6f92, 0x657d5bf0, 0x758ee421, 0x01234569, 0xfedbca98, 0x89abde01,
    0x76543213, 0xfedcba9b, 0x579a6f93, 0x657d5bf1, 0x758ee422, 0x0123456a, 0xfedbca99, 0x89abde02,
    0x76543214, 0xfedcba9c, 0x579a6f94, 0x657d5bf2, 0x758ee423, 0x0123456b, 0xfedbca9a, 0x89abde03,
    0x76543215, 0xfedcba9d, 0x579a6f95, 0x657d5bf3, 0x758ee424, 0x0123456c, 0xfedbca9b, 0x89abde04,
    0x76543216, 0xfedcba9e, 0x579a6f96, 0x657d5bf4, 0x758ee425, 0x0123456d, 0xfedbca9c, 0x89abde05,
    0x76543217, 0xfedcba9f, 0x579a6f97, 0x657d5bf5, 0x758ee426, 0x0123456e, 0xfedbca9d, 0x89abde06,
    0x76543218, 0xfedcbaa0, 0x579a6f98, 0x657d5bf6, 0x758ee427, 0x0123456f, 0xfedbca9e, 0x89abde07,

    0x76543219, 0xfedcbaa1, 0x579a6f99, 0x657d5bf7, 0x758ee428, 0x01234570, 0xfedbca9f, 0x89abde08,
    0x7654321a, 0xfedcbaa2, 0x579a6f9a, 0x657d5bf8, 0x758ee429, 0x01234571, 0xfedbcaa0, 0x89abde09,
    0x7654321b, 0xfedcbaa3, 0x579a6f9b, 0x657d5bf9, 0x758ee42a, 0x01234572, 0xfedbcaa1, 0x89abde0a,
    0x7654321c, 0xfedcbaa4, 0x579a6f9c, 0x657d5bfa, 0x758ee42b, 0x01234573, 0xfedbcaa2, 0x89abde0b,
    0x7654321d, 0xfedcbaa5, 0x579a6f9d, 0x657d5bfb, 0x758ee42c, 0x01234574, 0xfedbcaa3, 0x89abde0c,
    0x7654321e, 0xfedcbaa6, 0x579a6f9e, 0x657d5bfc, 0x758ee42d, 0x01234575, 0xfedbcaa4, 0x89abde0d,
    0x7654321f, 0xfedcbaa7, 0x579a6f9f, 0x657d5bfd, 0x758ee42e, 0x01234576, 0xfedbcaa5, 0x89abde0e,
    0x76543220, 0xfedcbaa8, 0x579a6fa0, 0x657d5bfe, 0x758ee42f, 0x01234577, 0xfedbcaa6, 0x89abde0f,

    0x76543221, 0xfedcbaa9, 0x579a6fa1, 0x657d5bff, 0x758ee430, 0x01234578, 0xfadbcaa7, 0x89abde10,
    0x76543222, 0xfedcbaaa, 0x579a6fa2, 0x657d5c00, 0x758ee431, 0x01234579, 0xfadbcaa8, 0x89abde11,
    0x76543223, 0xfedcbaab, 0x579a6fa3, 0x657d5c01, 0x758ee432, 0x0123457a, 0xfadbcaa9, 0x89abde12,
    0x76543224, 0xfedcbaac, 0x579a6fa4, 0x657d5c02, 0x758ee433, 0x0123457b, 0xfadbcaaa, 0x89abde13,
    0x76543225, 0xfedcbaad, 0x579a6fa5, 0x657d5c03, 0x758ee434, 0x0123457c, 0xfadbcaab, 0x89abde14,
    0x76543226, 0xfedcbaae, 0x579a6fa6, 0x657d5c04, 0x758ee435, 0x0123457d, 0xfadbcaac, 0x89abde15,
    0x76543227, 0xfedcbaaf, 0x579a6fa7, 0x657d5c05, 0x758ee436, 0x0123457e, 0xfadbcaad, 0x89abde16,
    0x76543228, 0xfedcbab0, 0x579a6fa8, 0x657d5c06, 0x758ee437, 0x0123457f, 0xfadbcaae, 0x89abde17,

    0x76543229, 0xfedcbab1, 0x579a6fa9, 0x657d5c07, 0x758ee438, 0x01234580, 0xfadbcab0, 0x89abde18,
    0x7654322a, 0xfedcbab2, 0x579a6faa, 0x657d5c08, 0x758ee439, 0x01234581, 0xfadbcab1, 0x89abde19,
    0x7654322b, 0xfedcbab3, 0x579a6fab, 0x657d5c09, 0x758ee43a, 0x01234582, 0xfadbcab2, 0x89abde1a,
    0x7654322c, 0xfedcbab4, 0x579a6fac, 0x657d5c0a, 0x758ee43b, 0x01234583, 0xfadbcab3, 0x89abde1b,
    0x7654322d, 0xfedcbab5, 0x579a6fad, 0x657d5c0b, 0x758ee43c, 0x01234584, 0xfadbcab4, 0x89abde1c,
    0x7654322e, 0xfedcbab6, 0x579a6fae, 0x657d5c0c, 0x758ee43d, 0x01234585, 0xfadbcab5, 0x89abde1d,
    0x7654322f, 0xfedcbab7, 0x579a6faf, 0x657d5c0d, 0x758ee43e, 0x01234586, 0xfadbcab6, 0x89abde1e,
    0x76543230, 0xfedcbab8, 0x579a6fb0, 0x657d5c0e, 0x758ee43f, 0x01234587, 0xfadbcab7, 0x89abde1f,

    0x76543231, 0xfedcbab9, 0x579a6fb1, 0x657d5c0f, 0x758ee440, 0x01234588, 0xfadbcab8, 0x89abde20,
    0x76543232, 0xfedcbaba, 0x579a6fb2, 0x657d5c10, 0x758ee441, 0x01234589, 0xfadbcab9, 0x89abde21,
    0x76543233, 0xfedcbabb, 0x579a6fb3, 0x657d5c11, 0x758ee442, 0x0123458a, 0xfadbcaba, 0x89abde22,
    0x76543234, 0xfedcbabc, 0x579a6fb4, 0x657d5c12, 0x758ee443, 0x0123458b, 0xfadbcabb, 0x89abde23,
    0x76543235, 0xfedcbabd, 0x579a6fb5, 0x657d5c13, 0x758ee444, 0x0123458c, 0xfadbcabc, 0x89abde24,
    0x76543236, 0xfedcbabe, 0x579a6fb6, 0x657d5c14, 0x758ee445, 0x0123458d, 0xfadbcabd, 0x89abde25,
    0x76543237, 0xfedcbabf, 0x579a6fb7, 0x657d5c15, 0x758ee446, 0x0123458e, 0xfadbcabe, 0x89abde26,
    0x76543238, 0xfedcbac0, 0x579a6fb8, 0x657d5c16, 0x758ee447, 0x0123458f, 0xfadbcabf, 0x89abde27,

    0x76543239, 0xfedcbac1, 0x579a6fb9, 0x657d5c17, 0x758ee448, 0x01234590, 0xfadbcac0, 0x89abde28,
    0x7654323a, 0xfedcbac2, 0x579a6fba, 0x657d5c18, 0x758ee449, 0x01234591, 0xfadbcac1, 0x89abde29,
    0x7654323b, 0xfedcbac3, 0x579a6fbb, 0x657d5c19, 0x758ee44a, 0x01234592, 0xfadbcac2, 0x89abde2a,
    0x7654323c, 0xfedcbac4, 0x579a6fbc, 0x657d5c1a, 0x758ee44b, 0x01234593, 0xfadbcac3, 0x89abde2b,
    0x7654323d, 0xfedcbac5, 0x579a6fbd, 0x657d5c1b, 0x758ee44c, 0x01234594, 0xfadbcac4, 0x89abde2c,
    0x7654323e, 0xfedcbac6, 0x579a6fbe, 0x657d5c1c, 0x758ee44d, 0x01234595, 0xfadbcac5, 0x89abde2d,
    0x7654323f, 0xfedcbac7, 0x579a6fbf, 0x657d5c1d, 0x758ee44e, 0x01234596, 0xfadbcac6, 0x89abde2e,
    0x76543240, 0xfedcbac8, 0x579a6fc0, 0x657d5c1e, 0x758ee44f, 0x01234597, 0xfadbcac7, 0x89abde2f,

    0x76543241, 0xfedcbac9, 0x579a6fc1, 0x657d5c1f, 0x758ee450, 0x01234598, 0xfadbcac8, 0x89abde30,
    0x76543242, 0xfedcbaca, 0x579a6fc2, 0x657d5c20, 0x758ee451, 0x01234599, 0xfadbcac9, 0x89abde31,
    0x76543243, 0xfedcbacb, 0x579a6fc3, 0x657d5c21, 0x758ee452, 0x0123459a, 0xfadbcaca, 0x89abde32,
    0x76543244, 0xfedcbacc, 0x579a6fc4, 0x657d5c22, 0x758ee453, 0x0123459b, 0xfadbcacb, 0x89abde33,
    0x76543245, 0xfedcbacd, 0x579a6fc5, 0x657d5c23, 0x758ee454, 0x0123459c, 0xfadbcacc, 0x89abde34,
    0x76543246, 0xfedcbace, 0x579a6fc6, 0x657d5c24, 0x758ee455, 0x0123459d, 0xfadbcacd, 0x89abde35,
    0x76543247, 0xfedcbacf, 0x579a6fc7, 0x657d5c25, 0x758ee456, 0x0123459e, 0xfadbcace, 0x89abde36,
    0x76543248, 0xfedcbad0, 0x579a6fc8, 0x657d5c26, 0x758ee457, 0x0123459f, 0xfadbcacf, 0x89abde37,

    0x76543249, 0xfedcbad1, 0x579a6fc9, 0x657d5c27, 0x758ee458, 0x012345a0, 0xfadbcad0, 0x89abde38,
    0x7654324a, 0xfedcbad2, 0x579a6fca, 0x657d5c28, 0x758ee459, 0x012345a1, 0xfadbcad1, 0x89abde39,
    0x7654324b, 0xfedcbad3, 0x579a6fcb, 0x657d5c29, 0x758ee45a, 0x012345a2, 0xfadbcad2, 0x89abde3a,
    0x7654324c, 0xfedcbad4, 0x579a6fcc, 0x657d5c2a, 0x758ee45b, 0x012345a3, 0xfadbcad3, 0x89abde3b,
    0x7654324d, 0xfedcbad5, 0x579a6fcd, 0x657d5c2b, 0x758ee45c, 0x012345a4, 0xfadbcad4, 0x89abde3c,
    0x7654324e, 0xfedcbad6, 0x579a6fce, 0x657d5c2c, 0x758ee45d, 0x012345a5, 0xfadbcad5, 0x89abde3d,
    0x7654324f, 0xfedcbad7, 0x579a6fcf, 0x657d5c2d, 0x758ee45e, 0x012345a6, 0xfadbcad6, 0x89abde3e,
    0x76543250, 0xfedcbad8, 0x579a6fd0, 0x657d5c2e, 0x758ee45f, 0x012345a7, 0xfadbcad7, 0x89abde3f,

    0xAAAAAAAA
};

// End buffer (where what is read is stored and then to be modified with flash_write_data)
uint32_t flash_read_data[1025]; // To read second sector (for verification)

#define BYTES_TO_WRITE 4100
#define WORDS_TO_WRITE ((BYTES_TO_WRITE + 3) / 4) // To deal with non-multiple of 4 bytes

// Buffer in flash where we have to write
int32_t __attribute__((section(".xheep_data_flash_only"))) __attribute__ ((aligned (16))) flash_write_buffer[WORDS_TO_WRITE]; 

// flash buffer address
uint32_t *f_address = flash_write_buffer;
// ram w/ data to write
uint32_t *rw_address = flash_write_data;
// ram w/ data to read
uint32_t *rr_address = flash_read_data;


// Check function
uint32_t check_result(uint8_t *test_buffer, uint32_t len);
uint32_t check_modify(uint32_t *test_buffer, uint32_t len);

__attribute__((optimize("O0"))) void my_ip_wflash(){
    // Gives the address offset how where the test_buffer is stored in the flash
    // r_address = heep_get_flash_address_offset(r_data);
    // Load r_address
    write_register( (uint32_t)f_address,
                    MY_IP_R_ADDRESS_REG_OFFSET,
                    0xFFFFFFFF,
                    0,
                    MY_IP_START_ADDRESS
                );
    // Load s_address
    write_register( (uint32_t)rr_address,
                    MY_IP_S_ADDRESS_REG_OFFSET,
                    0xFFFFFFFF,
                    0,
                    MY_IP_START_ADDRESS
                );
    // Load where to write from
    write_register( (uint32_t)rw_address,
                    MY_IP_MD_ADDRESS_REG_OFFSET,
                    0xFFFFFFFF,
                    0,
                    MY_IP_START_ADDRESS
                );
    // Load length
    write_register( BYTES_TO_WRITE,
                    MY_IP_LENGTH_REG_OFFSET,
                    0xFFFFFFFF,
                    0,
                    MY_IP_START_ADDRESS
                );
    // Specify it is a write operation
    write_register( 0x0,
                    MY_IP_CONTROL_REG_OFFSET,
                    0x1,
                    MY_IP_CONTROL_RNW_BIT,
                    MY_IP_START_ADDRESS
                );
    // Start read operation
    write_register( 0x1,
                    MY_IP_CONTROL_REG_OFFSET,
                    0x1,
                    MY_IP_CONTROL_START_BIT,
                    MY_IP_START_ADDRESS
                );
}

__attribute__((optimize("O0"))) void my_ip_rflash(){
    // Verify the write operation
    write_register( (uint32_t)f_address,
                    MY_IP_R_ADDRESS_REG_OFFSET,
                    0xFFFFFFFF,
                    0,
                    MY_IP_START_ADDRESS
                );
    // Load s_address
    write_register( (uint32_t)rr_address,
                    MY_IP_S_ADDRESS_REG_OFFSET,
                    0xFFFFFFFF,
                    0,
                    MY_IP_START_ADDRESS
                );
    // Load length
    write_register( BYTES_TO_WRITE,
                    MY_IP_LENGTH_REG_OFFSET,
                    0xFFFFFFFF,
                    0,
                    MY_IP_START_ADDRESS
                );
    // Specify it is a read operation
    write_register( 0x1,
                    MY_IP_CONTROL_REG_OFFSET,
                    0x1,
                    MY_IP_CONTROL_RNW_BIT,
                    MY_IP_START_ADDRESS
                );
    // Start read operation
    write_register( 0x1,
                    MY_IP_CONTROL_REG_OFFSET,
                    0x1,
                    MY_IP_CONTROL_START_BIT,
                    MY_IP_START_ADDRESS
                );
}

__attribute__((optimize("O0"))) void my_ip_run(){
    spi_host_t* spi;
    spi = spi_flash;

    if (w25q128jw_init(spi) != FLASH_OK) return EXIT_FAILURE;

    //printf("buffer_data[0] before anything!: 0x%08x\n",flash_read_data[0]);
    //printf("buffer_data[1] before anything: 0x%08x\n",flash_read_data[1]);
    //printf("buffer_data[-1] before anything: 0x%08x\n",flash_read_data[1023]);
    //printf("buffer_data[+1] before anything: 0x%08x\n",flash_read_data[1024]);

    my_ip_wflash();

    // Wait for DMA transfer to complete (write operation is completed)
    while(!my_ip_is_ready());

    printf("buffer_data[0] after write: 0x%08x\n",flash_read_data[0]);
    //printf("buffer_data[1] after write: 0x%08x\n",flash_read_data[1]);
    //printf("buffer_data[-1] after write: 0x%08x\n",flash_read_data[1023]);
    printf("buffer_data[+1] after write: 0x%08x\n",flash_read_data[1024]);

    // Can be quite long if WORDS_TO_WRITE is big
    // uint32_t res =  check_modify(flash_write_data, WORDS_TO_WRITE);

    my_ip_rflash();

    // Wait for read to verify to be completed
    while(!my_ip_is_ready());

    printf("buffer_data[0] after read: 0x%08x\n",flash_read_data[0]);
    //printf("buffer_data[1] after read: 0x%08x\n",flash_read_data[1]);
    //printf("buffer_data[-1] after read: 0x%08x\n",flash_read_data[1023]);
    printf("buffer_data[+1] after read: 0x%08x\n",flash_read_data[1024]);
}

int main(void) {

    my_ip_run();

    uint32_t res =  check_result(flash_write_data, BYTES_TO_WRITE);

    if (res == 0){
        return EXIT_SUCCESS;
    } else {
        return EXIT_FAILURE;
    }
}

uint32_t check_modify(uint32_t *expected_data, uint32_t len_words) {
    uint32_t errors = 0;

    // Verify flash_read_data (sector buffer) has correct modified data form flash_write_data
    for (uint32_t i = 0; i < len_words; i++) {
        if (expected_data[i] != flash_read_data[i]) {
            printf("Error at word %u: expected 0x%08x, got 0x%08x\n", 
                    i, expected_data[i], flash_read_data[i]);
            errors++;
        }
    }

    // Verify rest of flash_read_data is unchanged (0x00000000)
    for (uint32_t i = len_words; i < 1024; i++) {
        if (flash_read_data[i] != 0x00000000) {
            printf("Error at word %u: expected 0, got 0x%08x\n", 
                    i, flash_read_data[i]);
            errors++;
            break;  // Stop at first non-zero
        }
    }

    return errors;
}

uint32_t check_result(uint8_t *test_buffer, uint32_t len) {
    uint32_t errors = 0;
    uint8_t *flash_read_data_char = (uint8_t *)flash_read_data;

    for (uint32_t i = 0; i < len; i++) {
        if (test_buffer[i] != flash_read_data_char[i]) {
            printf("Error at position %d: expected %x, got %x\n", i, test_buffer[i], flash_read_data_char[i]);
            errors++;
            break;
        }
    }

    return errors;
}

// QUESTASIM SIM DURATION (BYTES_TO_WRITE 1028): 27.000.000 ns