...
// ram w/ data to write
uint32_t *rw_address = flash_write_data;
// ram w/ data to read
uint32_t *rr_address = flash_read_data;
...
__attribute__((optimize("O0"))) void my_ip_wflash(){
    ...
    // Load where to write from
    write_register( (uint32_t)rw_address,
                    MY_IP_MD_ADDRESS_REG_OFFSET,
                    0xFFFFFFFF,
                    0,
                    MY_IP_START_ADDRESS
                );
    ...
    // Specify it is a write operation
    write_register( 0x0,
...
// QUESTASIM SIM DURATION (BYTES_TO_WRITE 1028): 27.000.000 ns